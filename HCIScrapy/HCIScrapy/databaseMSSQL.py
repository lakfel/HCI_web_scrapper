...
from HCIScrapy.config import CONNECTION_STRING
import pandas as pd
    ...
    def get_random_issue_queries(cls, trial, num_records):
        try:
            ...
            FROM ISSUES_QUERY iq INNER JOIN ISSUES iss
                ON iq.doi = iss.doi
            WHERE id_trial = ? AND iss.status = 'OK'
            ORDER BY NEWID()
            """
            ...
            for record in records:
                cleaned_title = BeautifulSoup(record[1], "html.parser").get_text().replace('\n', ' ')
                cleaned_abstract = BeautifulSoup(record[2], "html.parser").get_text().replace('\n', ' ')
                records_t.append([record[0], cleaned_title, cleaned_abstract, *record[3:]])
